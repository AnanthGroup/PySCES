...
from tools import parse_xyz_data, assert_dictionary, cleanup, reset_directory, ArrayCompareMessage
...
class Test_TC_CIS(unittest.TestCase):
    ...
    def test_jobs(self):
        reset_directory()
        os.chdir('test_tc_interp')

        pysces.qcRunners.TeraChem._SAVE_BATCH = 'batches/'
        ...
        for file in ['corr.txt', 'electric_pq.txt', 'energy.txt', 'grad.txt', 'nac.txt']:
            ...
            for key in data_ref:
                np.testing.assert_allclose(data_tst[key], data_ref[key], 
                                           rtol=1e-4, verbose=True,
                                           err_msg = ArrayCompareMessage(data_tst[key], data_ref[key]))
                                        #    err_msg=f'file: {file}')
