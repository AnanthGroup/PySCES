...
def get_qc_runner(atoms, AN_mat):
    if opts.qc_runner == 'terachem':
        ...
        return qc_runner
    elif opts.qc_runner == 'gamess':
        from pysces.qcRunners.Gamess import GamessRunner
        qc_runner = GamessRunner(atoms, AN_mat)
        return qc_runner
    elif isinstance(opts.qc_runner, QCRunner):
        return opts.qc_runner
    else:
        sys.exit("Invalid QC Runner. Exitting.")
